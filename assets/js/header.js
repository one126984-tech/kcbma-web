...
(function() {
  ...
  const headerHtml = `
    <style>
      /* 긴급SOS 버튼: 시선을 끄는 빨간색 배경의 버튼 형태로 완벽하게 변경 */
      .sos-btn {
        background-color: #ef4444; /* 눈에 띄는 밝은 빨간색 */
      ...
      .sos-btn:hover {
        background-color: #dc2626; /* 마우스 오버 시 조금 더 짙은 빨간색 */
    ...
    <div class="w-full flex flex-col bg-white border-b border-gray-200 shadow-sm">
      <header class="flex justify-between items-center px-4 md:px-8 max-w-[1400px] mx-auto w-full h-[70px]">
        ...
        <!-- 2) 중앙 메인 GNB (PC 메뉴) -->
        <nav class="hidden lg:flex flex-1 justify-center lg:justify-between items-center px-6 xl:px-12 font-medium text-[15px] xl:text-[16px] text-gray-700 whitespace-nowrap">
          ...
          <div data-menu="community" data-href="${basePath}pages/community/field-experience.html" onclick="handleNavigation(this)" class="gnb-item cursor-pointer hover:text-[#0088cc] pb-1 transition-colors">자료 / 커뮤니티</div>

          <!-- 긴급SOS 호출 (경로 및 디자인 수정) -->
          <div data-menu="sos" data-href="${basePath}pages/SOS/sos-request.html" onclick="handleNavigation(this)" class="sos-btn hover:animate-none">
        ...
        <!-- 3) 우측 유틸리티 영역 -->
        <div class="hidden sm:flex items-center gap-3 shrink-0 ml-2 whitespace-nowrap font-medium text-gray-700">
          <div class="relative cursor-pointer flex items-center justify-center w-[36px] h-[36px] rounded-full hover:bg-gray-100 transition-colors" title="알림 확인">
            <i class="fa-regular fa-bell text-gray-700 text-[18px]"></i>
            <span class="absolute top-[2px] right-[2px] flex h-4 w-4 items-center justify-center rounded-full bg-red-500 text-[10px] font-bold text-white border-2 border-white">2</span>
          </div>
          
          <div data-href="${basePath}pages/mypage/dashboard.html" onclick="handleNavigation(this)" class="cursor-pointer text-[14px] hover:text-[#0088cc]">마이페이지</div>
          <div onclick="window.location.href='${basePath}login.html'" class="login-btn">로그인</div>
        </div>
    ...
    <div id="mobile-menu" style="display:none;" class="fixed inset-0 z-[1100] bg-black/60 transition-opacity" role="dialog" aria-modal="true">
      <div class="absolute top-0 right-0 bottom-0 w-4/5 max-w-[320px] bg-[#0a1931] shadow-2xl flex flex-col">
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-6 text-white font-medium text-[16px]">
          ...
          <div data-href="${basePath}pages/community/field-experience.html" onclick="handleNavigation(this)" class="cursor-pointer hover:text-[#0088cc]">자료 / 커뮤니티</div>
          
          <!-- 모바일 긴급SOS 호출 -->
          <div data-href="${basePath}pages/SOS/sos-request.html" onclick="handleNavigation(this)" class="sos-btn sos-btn-mobile">
            <span class="text-lg">🚨</span>
  ...
  // HTML 태그에 translate="no" 강제 추가 (스크립트 레벨 안전장치)
